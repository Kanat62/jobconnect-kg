  ...
  if (!job) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">Возможно, она была закрыта или удалена</p>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-20 bg-background border-b border-border px-4 py-3 flex items-center gap-3">
        ...
        <span className="flex-1 font-semibold text-sm truncate">Детальная страница</span>
        <button onClick={() => setIsFav(!isFav)} className="p-1.5 rounded-xl hover:bg-muted transition-colors">
          <Heart size={20} className={isFav ? "fill-primary text-primary" : "text-muted-foreground"} />
        </button>
        ...
        <div>
          ...
          {job.salary && (
            <p className="mt-3 text-xl font-bold text-foreground">
              {job.salary}
              {job.salaryPeriod && <span className="text-sm font-normal text-muted-foreground ml-1">{job.salaryPeriod}</span>}
            </p>
          )}
        </div>
